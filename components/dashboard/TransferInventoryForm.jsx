...
export default function TransferInventoryForm() {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Transfer Stock Successfully");
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-5xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 mx-auto my-3">
      <div className="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6">
        <TextInput label="Enter Quantity of Stock to Transfer" name="transferStockQty" register={register} errors={errors} type="number" />
        <SelectInput name="givingWarehouse" label="Select the Warehouse that will give the Stock" register={register} className="w-full" options={branches} />
        <SelectInput name="recievingWarehouse" label="Select the Warehouse that will receive the Stock" register={register} className="w-full" options={branches} />
